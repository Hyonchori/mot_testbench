...
def associate_byte(
    ...
    tracked_trk_indices = [i for i, trk in enumerate(trk_list) if trk.is_confirmed() and trk.is_lost()]
...
def associate_test(
    ...
    # first association
    cost_mat_iou, gate_mat_iou = get_iou_cost(trk_list, det_list, unmatched_trk_indices, unmatched_det_indices)
    # if img is not None:
    #     plot_utils.plot_cost(img, trk_list, det_list,
    #                          [cost_mat_iou, cost_mat_cosine],
    #                          [gate_mat_iou, gate_mat_cosine])

    matched_det_indices1, matched_trk_indices1 = linear_assignment(cost_mat_iou, gate_mat_iou)

    matched_det_indices1 = [unmatched_det_indices[det_idx] for det_idx in matched_det_indices1]
    matched_trk_indices1 = [unmatched_trk_indices[trk_idx] for trk_idx in matched_trk_indices1]

    unmatched_trk_indices = list(set(unmatched_trk_indices) - set(matched_trk_indices1))
    unmatched_det_indices = list(set(unmatched_det_indices) - set(matched_det_indices1))

    # second association
    # cost_mat_cosine, gate_mat_cosine = get_cosine_cost(trk_list, det_list, unmatched_trk_indices, unmatched_det_indices,
    #                                                    cost_thr=0.5)
    # matched_det_indices2, matched_trk_indices2 = linear_assignment(cost_mat_cosine, gate_mat_cosine)
    #
    # matched_det_indices2 = [unmatched_det_indices[det_idx] for det_idx in matched_det_indices2]
    # matched_trk_indices2 = [unmatched_trk_indices[trk_idx] for trk_idx in matched_trk_indices2]
    #
    # unmatched_trk_indices = list(set(unmatched_trk_indices) - set(matched_trk_indices2))
    # unmatched_det_indices = list(set(unmatched_det_indices) - set(matched_det_indices2))
    #
    # matched_det_indices = matched_det_indices1 + matched_det_indices2
    # matched_trk_indices = matched_trk_indices1 + matched_trk_indices2
    matched_det_indices = matched_det_indices1 #+ matched_det_indices2
    matched_trk_indices = matched_trk_indices1 #+ matched_trk_indices2
    matched_idx_pairs = [
        (det_idx, trk_idx) for det_idx, trk_idx in zip(matched_det_indices, matched_trk_indices)
