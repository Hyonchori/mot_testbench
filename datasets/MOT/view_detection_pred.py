# Visualize tracking prediction of MOT(17, 20) dataset
import argparse
import os
...
def main(args):
    ...
    for vid_idx, vid_name in enumerate(vid_list):
        ...
        pred_files = [x['-'.join(vid_name.split('-')[:2])] for x in pred_list]
        pred_paths = [os.path.join(pred_dir, pred_file) for pred_dir, pred_file in zip(pred_dirs.values(), pred_files)]
        preds = [parsing_mot_detection(pred_path) for pred_path in pred_paths]
        ...
        while img_idx < total_len:
            ...
            for j, (detector_name, pred) in enumerate(zip(pred_dirs.keys(), preds)):
                tmp_img = img.copy()
                plot_info(tmp_img, f'{detector_name}: {vid_name}: {img_idx + 1} / {len(imgs)}',
                          font_size=2, font_thickness=2)
                bboxes = pred[img_idx]
                for bbox in bboxes:
                    conf = float(bbox[4])
                    cls = int(float(bbox[5]))
                    cls_color = [int(x * conf) for x in colors(cls, True)]
                    xywh = np.array(list(map(float, bbox[0: 4])))
                    xyxy = xywh2xyxy(xywh)
                    cv2.rectangle(tmp_img, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), cls_color, 2)
                    plot_label(tmp_img, xyxy, f'{cls}: {conf:.2f}', cls_color, font_size=1)
